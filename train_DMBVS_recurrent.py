...
def ip_gen_our(frames, frames_op, start_id, device= "cuda", return_orig= False):
    if len(frames_op) < 3:
        if len(frames_op) == 0:
            frames_op.append(frames[start_id].clone())
            frames_op.append(frames[start_id + 1].clone())
        if len(frames_op) == 1:
...
def ip_gen_our_for_outer(frames, frames_op, frames_lbl, start_id, device= "cuda", return_orig= True, resize= True):
    if len(frames_op) < 3:
        if len(frames_op) == 0:
            frames_op.append(frames[start_id].clone())
            frames_op.append(frames[start_id + 1].clone())
...
def ip_gen_our_test(frames, frames_op, start_id, device= "cuda", patch_size= 330):
    ...
    if len(frames_op) < 3:
        if len(frames_op) == 0:
            frames_op.append(utils.tensor_crop(frames[start_id], h_, w_, patch_size).clone())
            frames_op.append(utils.tensor_crop(frames[start_id + 1], h_, w_, patch_size).clone())
        if len(frames_op) == 1:
...
def test_no_adaptation(db, net, device, L_in, epoch, opts):
    ...
    while(frame_pointer < len(frames) - 5 - 5):        
        with torch.no_grad():
            net.eval()
            ip_ = ip_gen_our(frames, interim_ops, frame_pointer, return_orig= False)
...
def test(db, net, device, L_in, epoch, opts):
    ...
    while(frame_pointer < len(frames) - 5 - 5):
        ...
        for _ in range(adaptation_number):
            optimizer.zero_grad()
            optimizer.zero_grad()
            ip1, spt_ip1 = ip_gen_our_test(frames, interim_op, start_id= frame_pointer, patch_size= opts.crop_size_adap)
            spt_op1 = torch.clamp(net(ip1), min= 0.0, max= 1.0)
            interim_op.append(spt_op1.clone())
            
            ip2, spt_ip2 = ip_gen_our_test(frames, interim_op, start_id= frame_pointer + 1, patch_size= opts.crop_size_adap)
            spt_op2 = torch.clamp(net(ip2), min= 0.0, max= 1.0)
            interim_op.append(spt_op2.clone())

            ip3, spt_ip3 = ip_gen_our_test(frames, interim_op, start_id= frame_pointer + 2, patch_size= opts.crop_size_adap)
            spt_op3 = torch.clamp(net(ip3), min= 0.0, max= 1.0)
            interim_op.append(spt_op3.clone())

            ip4, spt_ip4 = ip_gen_our_test(frames, interim_op, start_id= frame_pointer + 3, patch_size= opts.crop_size_adap)
            spt_op4 = torch.clamp(net(ip4), min= 0.0, max= 1.0)
            interim_op.append(spt_op4.clone())

            ip5, spt_ip5 = ip_gen_our_test(frames, interim_op, start_id= frame_pointer + 4, patch_size= opts.crop_size_adap)
    ...
    while(frame_pointer < len(frames) - 5 - 5): 
        with torch.no_grad():
            net.eval()
            ip_ = ip_gen_our(frames, interim_ops, frame_pointer)
            op_ = torch.clamp(net(ip_), min= 0.0, max= 1.0)
            interim_ops.append(op_.clone())
            final_op_list.append((utils.tensor2img(op_)*255).astype(np.uint8))
...
def train(data_loader, net, device, meta_opt, L_in, L_out, epoch, opts):
    ...
    for batch_idx, batch in enumerate(data_loader, 1):
        ...
        frame_inds = [0, 5, 11, 17, 23] #### sampling disjoint frames for training 5 inner loop iterations
        ...
        for i in range(1):
            with higher.innerloop_ctx(
                ...
                for ind in range(n_inner_iter):
                    interim_op = []
                    ip1, unstab1 = ip_gen_our(frames, interim_op, start_id= frame_inds[ind], return_orig= True)
                    spt_op1 = torch.clamp(fnet(ip1), min= 0.0, max= 1.0)
                    interim_op.append(spt_op1.clone())

                    ip2, unstab2 = ip_gen_our(frames, interim_op, start_id= frame_inds[ind] + 1, return_orig= True)
                    spt_op2 = torch.clamp(fnet(ip2), min= 0.0, max= 1.0)
                    interim_op.append(spt_op2.clone())

                    ip3, unstab3 = ip_gen_our(frames, interim_op, start_id= frame_inds[ind] + 2, return_orig= True)
                    spt_op3 = torch.clamp(fnet(ip3), min= 0.0, max= 1.0)
                    interim_op.append(spt_op3.clone())

                    ip4, unstab4 = ip_gen_our(frames, interim_op, start_id= frame_inds[ind] + 3, return_orig= True)
                    spt_op4 = torch.clamp(fnet(ip4), min= 0.0, max= 1.0)
                    interim_op.append(spt_op4.clone())

                    ip5, unstab5 = ip_gen_our(frames, interim_op, start_id= frame_inds[ind] + 4, return_orig= True)
                ...
                for q in range(6):
                    ...
                    if LBLS_GIVEN:
                        ip_qry, stab = ip_gen_our_for_outer(frames, interim_op, frames_lbl, start_id= id_for_qry)
                    else:
                        ip_qry, stab = ip_gen_our_for_outer(frames, interim_op, frames, start_id= id_for_qry)
                    op_qry.append(torch.clamp(fnet(ip_qry), min= 0.0, max= 1.0))
                    interim_op.append(op_qry[-1].clone())
...
if __name__ == "__main__":
    ...
    parser.add_argument('-model_name',      type=str,     default='another_recurrent_try',          help='path to save model')
    ...
    parser.add_argument('-crop_size_adap',  type=int,     default=256,                              help='adaptation patch size')
    ...
    test_no_adaptation(test_data_loader, model, device, L_in, 0, opts)
    torch.cuda.empty_cache()
