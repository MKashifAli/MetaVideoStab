...
def save_model(model, optimizer, opts, epoch):
...
def load_base_model(model, optimizer, opts, path):
    ...
    model.epoch = 34 ## reset model epoch
...
def learning_rate_decay(opts, epoch):
    ...
    lr = max(lr, opts.lr_init * opts.lr_min_m)

    return lr

def learning_rate_decay_simple(opts, epoch):
    ...
    if opts.lr_drop == 0: # constant learning rate
        ...
        assert(opts.lr_step > 0)
        if epoch%opts.lr_step == 0:
            decay = 1 ## decay = 1 for the first lr_offset iterations
        else:
            decay = 0
...
def batched_tensor_to_img_list(batch):
    b, c, h, w = batch.shape
    imgs = []
    for i in range(b):
        imgs.append(tensor2img(batch[i, :, :, :])*255.0)
    return imgs

def tensor_crop(batched_tensor, h, w, ps= 120):
    return batched_tensor[:, :, h:h+ps, w:w+ps]
