...
if __name__ == '__main__':
    ...
    spynet = SpyNet(model_name= 'chairs-final', weights_dir= "./models_spynet/", n_levels= 5).cuda().eval()
    
    arguments_strOne = './models_pwcnet/p0.png'
    arguments_strTwo = './models_pwcnet/p1.png'
    ...
    numpy.save("./models_spynet/flow_pro_spynet.npy", np_flow)


    arguments_strOne = './models_pwcnet/r0.png'
    arguments_strTwo = './models_pwcnet/r1.png'
    tenOne = torch.FloatTensor(numpy.expand_dims(cv2.imread(arguments_strOne)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0), 0)).cuda()
    tenTwo = torch.FloatTensor(numpy.expand_dims(cv2.imread(arguments_strTwo)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0), 0)).cuda()
    ...
    #print(tenOutput.shape)
    np_flow = numpy.transpose(tenOutput.numpy()[0, :, :, :], (1, 2, 0))
    numpy.save("./models_spynet/flow_raw_spynet.npy", np_flow)