...
from .correlation_package_pwc.correlation import Correlation 
import numpy as np
...
__all__ = [
    'pwc_dc_net', 'pwc_dc_net_new', 'pwc_dc_net_old'
    ]
...
class PWCDCNet(nn.Module):
    ...
    def forward(self,x):
        ...
        flow2 = flow2 * 20.0
        flow2 = F.interpolate(flow2, size= (h, w), mode= 'bilinear')
        flow2[:, 0, :, :] = flow2[:, 0, :, :] * (w/float(w_))
        flow2[:, 1, :, :] = flow2[:, 1, :, :] * (h/float(h_))
        #if self.training:
...
class PWCDCNet_old(nn.Module):
    ...
    def forward(self,x):
        ...
        x = F.interpolate(x, size= (h_, w_), mode= 'bicubic')
        ...
        flow2 = flow2 + self.dc_conv7(self.dc_conv6(self.dc_conv5(x)))
        flow2 = flow2 * 20.0
        flow2 = F.interpolate(flow2, size= (h, w), mode= 'bicubic')
        flow2[:, 0, :, :] = flow2[:, 0, :, :] * (w/float(w_))
        flow2[:, 1, :, :] = flow2[:, 1, :, :] * (h/float(h_))
        #if self.training:
        #    return flow2,flow3,flow4,flow5,flow6
...
class PWCDCNetNew(nn.Module):
    ...
    def __init__(self, md=4):
        ...
        """
        super(PWCDCNetNew,self).__init__()
...
if __name__ == '__main__':
    ...
    path = "./models_pwcnet/pwc_net.pth.tar"
    model = pwc_dc_net(path)
    model = model.cuda()
    ...
    with torch.no_grad():
        tenOutput = model(torch.cat([tenOne, tenTwo], dim= 1)) #estimate(tenOne, tenTwo)
    ...
    cv2.imwrite("./pwc_net_flow.png", fc)
